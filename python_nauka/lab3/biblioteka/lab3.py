...
#pobiera id szuka slownika po id ktory jest kluczem jak zmatchuje to usuwa jezeli nie to wywala blad ze nie znalazl
def usun_ksiazke(id):
    done = False
...
#pobiera tytul i biblioteke, szuka po tytule w ksiazkach jak zmatchuje dodaje do tymczasowej tablicy znalezionych ksiazek jezeli znajdzie weicej niz 1 to printuje wszystkie jezeli nie to printuje tylko 1 jezeli nie znajdzie zadnej to wywala blad
...
def oblicz_koszt_egzemplarzy(id, biblio):
    ...
    calkowita_cena = 0
    if ksiega:
        while i <= ksiega['quantity']:
            i += 1
            calkowita_cena += ksiega['price']
...
def oblicz_koszt_calej_biblioteki(biblio = {}):
    ...
    for ksiazka, data in biblio.items():
        calkowity_koszt_biblio += oblicz_koszt_egzemplarzy(ksiazka)
...
def main():  
    ...
    print("7 - zapisz aktualny stan biblioteki do pliku textowego")
    ...
    if num == 0:
        ...
        if oblicz_koszt_egzemplarzy(id, biblioteka):
            print(f'cena ksiazki o ID: {id} wynosi ={oblicz_koszt_egzemplarzy(id)}')
        else:
